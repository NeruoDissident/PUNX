...
function initializeGraph(nodes, edges) {
  try {
    ...
    if (window.showOnlyControl && typeof window.showOnlyControl.init === 'function') {
      try {
        window.showOnlyControl.init(cy);
      } catch (error) {
        console.error('Error initializing showOnlyControl:', error);
      }
    }
    
    if (window.showOnlyControl && typeof window.showOnlyControl.update === 'function') {
      try {
        window.showOnlyControl.update(cy);
      } catch (error) {
        console.error('Error updating showOnlyControl:', error);
      }
    }
    ...
    if (window.depthControl && typeof window.depthControl.setup === 'function') {
      try {
        window.depthControl.setup(cy);
      } catch (error) {
        console.error('Error setting up depth control:', error);
      }
    }
    
    // Setup cluster control events
    if (window.clusterControl && typeof window.clusterControl.setupEvents === 'function') {
      try {
        ...
        console.error('Error setting up cluster control events:', error);
      }
    }

    // Enable cluster dragging via clusterControl utility
    if (window.clusterControl && typeof window.clusterControl.setupDrag === 'function') {
      try {
        ...
        console.error('Error setting up cluster drag:', error);
      }
    }
